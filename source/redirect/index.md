  ...
  if (goto) {
    ...
    const url = new URL(goto);
        ...
        try {
          ...
          if (response.type === 'opaque' || url.protocol === 'https:') {
            protocol = '安全连接';
          } else {
            protocol = 'HTTP 普通连接';
          }
        } catch(e) {
          throw new Error('无法连接到目标网站');
        }    
        document.getElementById('target-info').innerHTML = `
          ...
          <img src="${url.origin}/favicon.ico" onerror="this.outerHTML='<svg xmlns=\'http://www.w3.org/2000/svg\' width=\'3em\' height=\'3em\' viewBox=\'0 0 24 24\'><path fill=\'currentColor\' d=\'M4 11v2h12l-5.5 5.5l1.42 1.42L19.84 12l-7.92-7.92L10.5 5.5L16 11z\'/></svg>'" style="width: 3em; height: 3em;">
        <p>您即将访问以下网站：</p>
        <!-- <p><strong>${url.hostname}</strong></p> -->
        <p><span style="cursor: pointer; text-decoration: underline;" onclick="navigator.clipboard.writeText('${goto}').then(() => alert('原始链接已复制到剪贴板！'))">${url.hostname}</span></p>    <p>连接类型：${protocol}</p>
